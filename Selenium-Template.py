...
curl_cmd = "curl 'http://localhost:8191/v1' -H 'Content-Type: application/json' --data '{\"cmd\": \"request.get\",\"url\":\"https://javgg.net/new-post\",\"maxTimeout\": 60000}'"
...
# 使用 subprocess 模块调用 curl 命令，并捕获命令输出结果
curl_cmd = "curl 'http://localhost:8191/v1' -H 'Content-Type: application/json' --data '{\"cmd\": \"request.get\",\"url\":\"https://supjav.com/category/censored-jav\",\"maxTimeout\": 60000}'"
result = subprocess.check_output(curl_cmd, shell=True)
